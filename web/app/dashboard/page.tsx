      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'Trends', value: trends.length, icon: PlusIcon, color: 'text-purple-400' },
            { label: 'Categories', value: stats?.top_categories?.length || 0, icon: ChartBarIcon, color: 'text-yellow-400' },
          ].map((stat, i) => (
        ...
        <div className="mt-6 flex gap-2">
          <button onClick={() => handleScrape('toolify')} className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg text-sm font-medium">
            Scrape Toolify
          </button>
          <button onClick={() => handleScrape('producthunt')} className="bg-orange-600 hover:bg-orange-700 px-4 py-2 rounded-lg text-sm font-medium">
            Scrape Product Hunt
          </button>
        </div>
      </main>
